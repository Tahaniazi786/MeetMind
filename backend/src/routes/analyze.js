...
router.post("/", async (req, res, next) => {
  try {
    ...
    console.log(
      `🧠 Analyzing transcript with Gemini: ${transcript.length} chars (${transcript.split(/\s+/).length} words)`
    );
    ...
    console.log(
      `✅ Gemini Analysis complete in ${elapsed}s — health score: ${analysis.meeting_health_score}/100, ${analysis.action_items.length} action items`
    );
