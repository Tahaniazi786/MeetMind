...
const { transcribeAudio } = require("../lib/geminiAudio");

const router = express.Router();

// ── Multer config — 25 MB limit, audio files ───────────────────
...
router.post("/", upload.single("audio"), async (req, res, next) => {
  ...
  try {
    ...
    console.log(`🎙️  Transcribing with Gemini: ${req.file.originalname} (${(req.file.size / 1024 / 1024).toFixed(2)} MB)`);
